...
void timed_event::actualize()
{
    ...
    switch( type ) {
        ...
        case timed_event_type::MORTAR_FIRE_MESSAGE: {
            ...
            const bool guided = round.typeId() == itype_81mm_shell_acerm ||
                                guided_round_uses_drone_designation( fire_data->ammo_id );
            const bool payload_scheduled = guided ?
                                           mortar_schedule_guided_impact_payload(
                                               round, map_square, fire_data->target, impact_time,
                                               fire_data->impact_message_strength, string_id ) :
                                           mortar_schedule_impact_payload( round, map_square,
                                                   impact_time );
            if( !payload_scheduled ) {
                add_msg_debug( debugmode::DF_NPC,
                               "Scheduled mortar fire canceled: ammunition has no impact payload." );
                break;
            }
            ...
            impact_data.feedback_reported = fire_data->feedback_reported;
            impact_data.report = !guided;
        ...
        case timed_event_type::MORTAR_QUEUED_FIRE: {
            const timed_event_character_data *queued_fire = get_data<timed_event_character_data>();
            if( queued_fire == nullptr ) {
                add_msg_debug( debugmode::DF_NPC,
                               "Queued mortar fire ignored: missing event payload." );
                break;
            }
            npc *gunner = queued_fire->character.is_valid() ? g->find_npc( queued_fire->character ) :
                          nullptr;
            if( gunner == nullptr ) {
                add_msg_debug( debugmode::DF_NPC,
                               "Queued mortar fire ignored: gunner id %d no longer exists.",
                               queued_fire->character.get_value() );
                break;
            }
        ...
        const int radius, const int age_seconds )
{
    ...
    field_data->radius = std::max( 0, radius );
    field_data->age_seconds = age_seconds;
}
