...
static int npc_mortar_support_menu()
{
    ...
    nmenu.addentry( NPC_CHAT_MORTAR_SUPPORT_FIRE_FOR_EFFECT, true, 'e',
                    _( "Fire for effect" ) );
...
void game::chat( const std::optional<tripoint_bub_ms> &p )
{
    ...
    const std::vector<npc *> available_mortar_support = get_npcs_if( [&]( const npc & guy ) {
        const diag_value mortar_assignment = guy.get_value( "mortar_assignment" );
        return guy.is_player_ally() && guy.can_hear( player_character.pos_bub(), volume ) &&
               guy.companion_mission_role_id != "FACTION_CAMP" &&
        ...
        case NPC_CHAT_MORTAR_SUPPORT: {
            ...
            switch( mortar_order ) {
                ...
                case NPC_CHAT_MORTAR_SUPPORT_REPEAT_FIRE:
                    ...
                    break;
                case NPC_CHAT_MORTAR_SUPPORT_FIRE_FOR_EFFECT:
                    talk_effect_fun::request_mortar_fire_for_effect( gunner );
                    break;
...
struct nearby_mortar_assignment {
    tripoint_bub_ms pos;
    const mortar_type *type = nullptr;
};

std::optional<nearby_mortar_assignment> find_nearby_mortar( const Character &center )
{
    map &here = get_map();
    const tripoint_bub_ms center_pos = center.pos_bub( here );
    std::optional<nearby_mortar_assignment> best;
    int best_dist = INT_MAX;
    for( const tripoint_bub_ms &pos : here.points_in_radius( center_pos, 24 ) ) {
        const mortar_type *mortar = mortar_type::from_furniture( here.furn( pos ).id() );
        if( mortar == nullptr ) {
            continue;
        }
        const int dist = rl_dist( center_pos, pos );
        if( dist < best_dist ) {
            best = nearby_mortar_assignment{ pos, mortar };
            best_dist = dist;
        }
    }
    return best;
...
bool is_mortar_round_for_type( const item &it, const mortar_type &mortar )
{
...
{
    int count = 0;
    for( const item_location &loc : gunner.cache_get_items_with( ammo_id ) ) {
        if( !loc || ( player_issued_only && !loc->has_var( mortar_support_ammo_var ) ) ) {
            continue;
        }
        count += loc->count_by_charges() ? loc->charges : 1;
    }
    return count;
}
...
void store_mortar_round( npc &gunner, item round )
{
    if( !gunner.i_add( round, true, nullptr, nullptr, false, false, true ) ) {
        gunner.inv->add_item( std::move( round ), false, true, false );
    }
...
std::string support_value_string( const npc &operator_npc, const std::string &key )
{
    const diag_value value = operator_npc.get_value( key );
    return value.is_empty() ? std::string() : value.str();
}

int support_value_int( const npc &operator_npc, const std::string &key )
{
    const diag_value value = operator_npc.get_value( key );
...
int count_support_items( const npc &operator_npc,
...
bool support_has_item_with_flag( const npc &operator_npc, const flag_id &flag )
{
    return count_support_items( operator_npc, [&flag]( const item & it ) {
        ...
        const std::function<bool( const item & )> &filter )
{
    for( int pos = 0; pos < static_cast<int>( source.size() ); ++pos ) {
        ...
        item taken = candidate;
        if( candidate.count_by_charges() ) {
            taken.charges = 1;
                                       ...
                                       const std::function<bool( const item & )> &filter )
{
    ...
    for( const std::pair<const std::string, int> &entry : counts ) {
        lines.emplace_back( string_format( "%s x%d", entry.first, entry.second ) );
    }
    return string_join( lines, ", " );
}

int give_support_items( npc &operator_npc, const std::function<bool( const item & )> &filter,
                        const std::string &title )
{
    int transferred = 0;
    drop_locations selected_items = select_nearby_handover_items( filter, title );
    for( drop_location &selected : selected_items ) {
        std::optional<item> transferred_item = take_handover_item( selected.first, selected.second );
...
void manage_support_inventory( npc &operator_npc,
                               const std::function<bool( const item & )> &filter, const std::string &menu_title,
                               const std::string &give_title, const std::string &take_title )
{
    while( true ) {
        const int action = uilist( menu_title, {
            _( "Provide equipment or ammunition." ),
            _( "Take equipment or ammunition back." ),
            _( "Ask what equipment and ammunition are available." )
        } );
        if( action < 0 ) {
            return;
        }
        if( action == 0 ) {
            const int transferred = give_support_items( operator_npc, filter, give_title );
            if( transferred > 0 ) {
                add_msg( _( "You hand %1$d item to %2$s for support duty." ), transferred,
                         operator_npc.disp_name() );
            }
        } else if( action == 1 ) {
            const int returned = take_back_support_items( operator_npc, filter, take_title );
            if( returned > 0 ) {
                add_msg( _( "%1$s returns %2$d support item." ), operator_npc.disp_name(), returned );
            }
        } else {
            add_msg( _( "%1$s reports available support equipment: %2$s." ),
                     operator_npc.disp_name(), support_inventory_summary( operator_npc, filter ) );
        }
    }
}
                        ...
                        const bool quiet = false )
{
    ...
    for( drop_location &selected : selected_rounds ) {
        ...
        if( !round || !is_mortar_round_for_type( *round, mortar ) ) {
            continue;
        }
        ...
        if( !stored_selected_mortar_ammo( gunner ) ) {
            set_selected_mortar_ammo( gunner, round->typeId() );
        }
...
std::string mortar_ammo_summary( const npc &gunner, const mortar_type &mortar )
{
    std::vector<std::string> lines;
    for( const itype_id &ammo_id : available_mortar_ammo_types( gunner, mortar ) ) {
...
int total_mortar_ammo_count( const npc &gunner, const mortar_type &mortar )
{
    int total = 0;
    for( const itype_id &ammo_id : available_mortar_ammo_types( gunner, mortar ) ) {
...
int take_back_mortar_rounds( npc &gunner, const mortar_type &mortar )
{
    ...
    for( const itype_id &ammo_id : mortar_ammo_types( gunner, true ) ) {
        if( is_mortar_round_for_type( item( ammo_id, calendar::turn ), mortar ) ) {
            ammo_types.emplace_back( ammo_id );
        }
    }
    if( ammo_types.empty() ) {
    ...
    for( size_t i = 0; i < ammo_types.size(); ++i ) {
        ...
        menu.addentry( i, true, MENU_AUTOASSIGN, "%s (%d)", round.tname(),
                       physical_mortar_ammo_count( gunner, ammo_types[i], true ) );
    }
    menu.query();
    ...
    avatar &you = get_avatar();
    int count = 0;
    for( item &round : returned_rounds ) {
        count += round.count_by_charges() ? round.charges : 1;
        round.erase_var( std::string( mortar_support_ammo_var ) );
        you.i_add_or_drop( round );
    }
    if( selected && *selected == ammo_id && mortar_ammo_count( gunner, ammo_id ) <= 0 ) {
...
std::string mortar_ammo_name( const mortar_type &mortar )
{
    return mortar.ammo().obj().name();
}

std::vector<itype_id> available_mortar_ammo_types( const npc &gunner, const mortar_type &mortar )
{
    std::vector<itype_id> result;
...
int release_mortar_ammo_impl( npc &gunner, const bool drop )
{
    ...
    if( drop ) {
        ...
        for( item &round : rounds ) {
            released += round.count_by_charges() ? round.charges : 1;
            round.erase_var( std::string( mortar_support_ammo_var ) );
...
{
    const diag_value &value = gunner.get_value( "mortar_adjustment_ready_turn" );
    if( value.is_empty() ) {
    ...
    const double ready_turn = value.dbl();
    return calendar::turn_zero + time_duration::from_turns( static_cast<int>( ready_turn ) );
}

void set_mortar_adjustment_ready_at( npc &gunner, const time_point &ready_at )
{
    gunner.set_value( "mortar_adjustment_ready_turn",
                      to_turns<int>( ready_at - calendar::turn_zero ) );
}
...
constexpr double mortar_drone_spotter_sensor_multiplier = 1.6;
constexpr double mortar_drone_detection_range_multiplier = 2.0;
...
{
    if( has_charged_radio( spotter ) && has_charged_radio( gunner ) ) {
        return mortar_report_mode_radio;
    }
    if( rl_dist( spotter.pos_abs(), gunner.pos_abs() ) <= MAX_VIEW_DISTANCE ) {
        return mortar_report_mode_shout;
    }
    return mortar_report_mode_none;
}
...
double mortar_perception_spotting_factor( const int perception )
{
    ...
    return std::min( 1.0, 0.5 + ( perception - 5 ) * ( 0.5 / 5.0 ) );
}

struct mortar_spotter_context {
    const Character *observer = nullptr;
    tripoint_abs_ms pos;
    std::string name;
    bool player = false;
    bool drone = false;
    int report_mode = mortar_report_mode_none;
    double sensor_multiplier = 1.0;
    double detection_range_multiplier = 1.0;
    double repeat_location_multiplier = 0.5;
};

double mortar_repeat_accuracy_multiplier( const mortar_type &mortar,
        const mortar_spotter_context &spotter, const int launcher_skill )
{
    const double base_reduction = 1.0 - mortar.repeat_cep_multiplier( launcher_skill );
    const double reduction = clamp( base_reduction *
                                    mortar_perception_spotting_factor( spotter.observer->get_per() ) *
                                    spotter.sensor_multiplier, 0.0, 0.95 );
    return 1.0 - reduction;
}

double mortar_feedback_multiplier( const double multiplier, const bool no_wait_adjustment )
{
    const double clamped = clamp( multiplier, 0.0, 1.0 );
    if( !no_wait_adjustment ) {
        return clamped;
    }
    return 1.0 - ( 1.0 - clamped ) * mortar_no_wait_feedback_reduction_scale;
}

double mortar_base_location_error( const int perception, const double sensor_multiplier )
{
    const double clamped_perception = clamp<double>( perception, 1.0, 10.0 );
    double base = 0.0;
    if( clamped_perception <= 5.0 ) {
        base = 500.0 - ( clamped_perception - 1.0 ) * ( 250.0 / 4.0 );
    } else {
        base = 250.0 - ( clamped_perception - 5.0 ) * ( 100.0 / 5.0 );
    }
    return std::max( 1.0, base * mortar_binocular_reference_multiplier /
                     sensor_multiplier );
}

double mortar_base_location_error( const mortar_spotter_context &spotter )
{
    return mortar_base_location_error( spotter.observer->get_per(), spotter.sensor_multiplier );
}

mortar_location_error mortar_make_location_error( const mortar_spotter_context &spotter,
        const tripoint_abs_ms &target, const double cep )
{
    if( spotter.player ) {
        return mortar_make_location_error( *spotter.observer, target, cep );
    }
    return mortar_location_error{ cep, cep };
}
...
std::optional<tripoint_abs_ms> mortar_drone_station_position( const npc &operator_npc )
{
    const auto make_position = []( const npc & operator_npc, const std::string & x_key,
    const std::string & y_key, const std::string & z_key ) -> std::optional<tripoint_abs_ms> {
        const diag_value x_value = operator_npc.get_value( x_key );
        const diag_value y_value = operator_npc.get_value( y_key );
        const diag_value z_value = operator_npc.get_value( z_key );
        if( x_value.is_empty() || y_value.is_empty() || z_value.is_empty() )
        {
            return std::nullopt;
        }
        return tripoint_abs_ms( static_cast<int>( x_value.dbl() ),
                                static_cast<int>( y_value.dbl() ),
                                static_cast<int>( z_value.dbl() ) );
    };

    if( support_value_string( operator_npc, "fpv_scout_active" ) == "yes" &&
        current_turn_number() >= support_value_int( operator_npc, "fpv_scout_ready_turn" ) ) {
        return make_position( operator_npc, "fpv_scout_x", "fpv_scout_y", "fpv_scout_z" );
    }
    if( support_value_string( operator_npc, "fpv_scout_report_active" ) == "yes" ) {
        return make_position( operator_npc, "fpv_scout_report_x", "fpv_scout_report_y",
                              "fpv_scout_report_z" );
    }
    return make_position( operator_npc, "fpv_station_x", "fpv_station_y", "fpv_station_z" );
}

bool mortar_drone_spotting_disabled( const npc &operator_npc )
{
...
enum class fpv_designation_target_kind : int {
    tile,
    character,
    monster
};

struct fpv_designation_target {
...
{
    switch( kind ) {
        case fpv_designation_target_kind::character:
            return "character";
        case fpv_designation_target_kind::monster:
            return "monster";
...
static void write_fpv_designation_target( npc &operator_npc, const std::string &designation_type,
        const fpv_designation_target &target )
{
    operator_npc.set_value( "fpv_designation_active", "yes" );
    operator_npc.set_value( "fpv_designation_type", designation_type );
    operator_npc.set_value( "fpv_designation_x", target.pos.x() );
    operator_npc.set_value( "fpv_designation_y", target.pos.y() );
    operator_npc.set_value( "fpv_designation_z", target.pos.z() );
    operator_npc.set_value( "fpv_designation_target_type",
                            fpv_designation_target_kind_name( target.kind ) );
    operator_npc.set_value( "fpv_designation_character_id",
                            target.target_character.get_value() );
    operator_npc.set_value( "fpv_designation_monster_id", target.target_monster );
}
    ...
    npc &operator_npc )
{
    ...
    if( target.kind == fpv_designation_target_kind::character ) {
        if( target.target_character == get_avatar().getID() ) {
            target_creature = &get_avatar();
        } else if( target.target_character.is_valid() ) {
            target_creature = g->find_npc( target.target_character );
        }
    } else if( target.kind == fpv_designation_target_kind::monster ) {
        shared_ptr_fast<monster> critter = get_creature_tracker().from_temporary_id(
    ...
    const std::optional<tripoint_abs_ms> drone_pos = mortar_drone_station_position( operator_npc );
    if( !drone_pos ) {
        return std::nullopt;
    }

    map &here = get_map();
    const tripoint_bub_ms drone_bub = here.get_bub( *drone_pos );
    const tripoint_bub_ms target_bub = target_creature->pos_bub( here );
    if( !here.inbounds( drone_bub ) || !here.inbounds( target_bub ) ) {
        return std::nullopt;
    }

    here.build_map_cache( drone_bub.z() );
    here.update_visibility_cache( drone_bub.z() );
    if( !here.sees( drone_bub, target_bub, rl_dist( drone_bub, target_bub ) + 3 ) ) {
        return std::nullopt;
    }

    target.pos = target_creature->pos_abs();
    operator_npc.set_value( "fpv_designation_x", target.pos.x() );
    operator_npc.set_value( "fpv_designation_y", target.pos.y() );
    operator_npc.set_value( "fpv_designation_z", target.pos.z() );
    return target;
...
{
    avatar &you = get_avatar();
    std::vector<mortar_spotter_context> spotters;
    spotters.emplace_back( mortar_spotter_context{
        &you,
        you.pos_abs(),
        _( "you" ),
        true,
        false,
        mortar_report_mode( you, gunner ),
        mortar_spotter_sensor_multiplier( you, target ),
        1.0,
        mortar_uses_laser_rangefinder( you, target ) ?
        mortar_laser_rangefinder_repeat_location_multiplier : 0.5
    } );

    std::vector<npc *> drone_operators = g->get_npcs_if( [&gunner]( const npc & guy ) {
        return &guy != &gunner && guy.is_player_ally() &&
               !support_value_string( guy, "fpv_assignment" ).empty();
    } );
    for( npc *operator_npc : drone_operators ) {
        reconcile_fpv_mission( *operator_npc );
        if( support_value_string( *operator_npc, "fpv_status" ) != "on_station" ||
            ...
            mortar_drone_spotting_disabled( *operator_npc ) ||
            mortar_report_mode( *operator_npc, gunner ) == mortar_report_mode_none ) {
            continue;
        }
        ...
        spotters.emplace_back( mortar_spotter_context{
            ...
            *drone_pos,
            ...
            false,
            true,
            mortar_report_mode( *operator_npc, gunner ),
            mortar_drone_spotter_sensor_multiplier,
...
const mortar_spotter_context &best_location_spotter( const std::vector<mortar_spotter_context>
        &spotters )
{
    return *std::min_element( spotters.begin(), spotters.end(),
    []( const mortar_spotter_context & lhs, const mortar_spotter_context & rhs ) {
        return mortar_base_location_error( lhs ) < mortar_base_location_error( rhs );
    } );
}

double get_mortar_location_error( const npc &gunner, const double fallback )
{
...
{
    gunner.set_value( "mortar_location_error", std::max( 1.0, error ) );
}

double mortar_near_retarget_location_error_rate( const int perception )
{
    if( perception <= 3 ) {
        return 0.5;
    }
    ...
    return 0.5 - ( perception - 3 ) * ( 0.3 / 7.0 );
}

double mortar_shot_lost_chance( const int perception, const double sensor_multiplier,
                                const tripoint_abs_ms &spotter_pos, const tripoint_abs_ms &impact,
                                const double detection_range_multiplier )
{
    const double clamped_perception = clamp<double>( perception, 1.0, 10.0 );
    double chance = 0.0;
    if( clamped_perception <= 5.0 ) {
        chance = 0.20 - ( clamped_perception - 1.0 ) * ( 0.10 / 4.0 );
    } else {
        chance = 0.10 - ( clamped_perception - 5.0 ) * ( 0.07 / 5.0 );
    }
    chance += 0.10 * rl_dist( spotter_pos, impact ) /
              ( 1000.0 * std::max( 0.1, detection_range_multiplier ) );
    chance /= sensor_multiplier;
    return clamp( chance, 0.0, 0.95 );
}

double mortar_shot_lost_chance( const mortar_spotter_context &spotter,
                                const tripoint_abs_ms &impact )
{
    if( spotter.player && !mortar_spotter_can_observe( *spotter.observer, impact ) ) {
        return 1.0;
    }
    return mortar_shot_lost_chance( spotter.observer->get_per(), spotter.sensor_multiplier,
                                    spotter.pos, impact, spotter.detection_range_multiplier );
}
...
mortar_error mortar_current_report_error( const npc &gunner, const assigned_mortar &mortar,
        const mortar_spotter_context &spotter, const tripoint_abs_ms &target )
{
    ...
    const double location_error_cep = get_mortar_location_error( gunner,
                                      mortar_base_location_error( spotter ) );
    ...
    return mortar.type->combined_error( mortar.pos, target, ballistic_error,
                                        spotter.pos, target, location_error );
}

mortar_error mortar_current_report_error( const npc &gunner, const assigned_mortar &mortar,
        const Character &spotter, const tripoint_abs_ms &target )
{
    const int target_distance = rl_dist( mortar.pos, target );
    const double accuracy_multiplier = get_mortar_accuracy_multiplier( gunner );
    const double sensor_multiplier = mortar_spotter_sensor_multiplier( spotter, target );
    const double location_error_cep = get_mortar_location_error( gunner,
                                      mortar_base_location_error( spotter.get_per(),
                                              sensor_multiplier ) );
    const mortar_location_error location_error = mortar_make_location_error( spotter, target,
            location_error_cep );
    const double fixed_multiplier = mortar_fixed_accuracy_multiplier( gunner, mortar.pos, false );
    ...
    const mortar_error ballistic_error{ minimum_error.range * total_multiplier,
                                        minimum_error.deflection * total_multiplier };
    return mortar.type->combined_error( mortar.pos, target, ballistic_error,
                                        spotter.pos_abs(), target, location_error );
}

struct mortar_fire_for_effect_readiness {
    mortar_error reported_error;
    double range_threshold = 0.0;
};

std::optional<mortar_fire_for_effect_readiness> mortar_fire_for_effect_readiness_for(
    npc &gunner, const assigned_mortar &mortar )
{
    const std::optional<tripoint_abs_ms> target = get_mortar_last_target( gunner );
    if( !target ) {
        return std::nullopt;
    }
    const std::vector<mortar_spotter_context> spotters =
        mortar_spotters_for_fire_mission( gunner, *target );
    const mortar_spotter_context &spotter = best_location_spotter( spotters );
    const int target_distance = rl_dist( mortar.pos, *target );
        ...
        const assigned_mortar &mortar, const Character &spotter )
{
    ...
    const mortar_error reported_error = mortar_current_report_error( gunner, mortar, spotter,
                                        *target );
...
void practice_mortar_shot( npc &gunner )
{
    ...
    gunner.practice_proficiency( proficiency_prof_mortar_operation, 5_minutes );
}
        ...
        while( true ) {
            ...
            if( action == 0 ) {
                ...
                add_msg( _( "%1$s reports available mortar ammunition: %2$s." ),
                         gunner->disp_name(), mortar_ammo_summary( *gunner, *mortar->type ) );
            }
        }
    };
}
...
std::optional<int> select_mortar_fire_for_effect_round_count( npc &gunner,
        const assigned_mortar &mortar )
{
    const std::optional<mortar_fire_for_effect_readiness> readiness =
        mortar_fire_for_effect_readiness_for( gunner, mortar );
    if( !readiness ) {
        add_msg( _( "%s reports they do not have a mortar target for fire for effect." ),
                 gunner.disp_name() );
        return std::nullopt;
    }

    add_msg_debug( debugmode::DF_NPC,
                   "Mortar fire for effect readiness for %s: range error %.2f, threshold %.2f, "
                   "ready %d.",
                   gunner.disp_name(), readiness->reported_error.range, readiness->range_threshold,
    ...
    uilist menu;
    menu.text = _( "Fire for effect: how many rounds?" );
    menu.addentry( 2, true, '2', _( "Fire 2 rounds" ) );
    menu.addentry( 4, true, '4', _( "Fire 4 rounds" ) );
    menu.addentry( 6, true, '6', _( "Fire 6 rounds" ) );
    menu.query();
    if( menu.ret != 2 && menu.ret != 4 && menu.ret != 6 ) {
        return std::nullopt;
    }
    return menu.ret;
}
...
{
    avatar &you = get_avatar();
    const int round_count = std::max( 1, requested_rounds );
    ...
    if( adjustment_pending || ( from_queue && fire_order_pending ) ) {
        const time_point queued_at = std::max( calendar::turn + 1_turns,
                                               mortar_adjustment_ready_at( gunner ) );
        queue_mortar_fire_order( gunner, *target_abs_ms, round_count, queued_at );
        return;
    }

    const std::vector<mortar_spotter_context> spotters =
        mortar_spotters_for_fire_mission( gunner, *target_abs_ms );
    const mortar_spotter_context &location_spotter = best_location_spotter( spotters );
    const bool eplrs_net_available = mortar_uses_eplrs_net( you, gunner );
    const bool laser_rangefinder_used = location_spotter.player &&
                                        mortar_uses_laser_rangefinder( *location_spotter.observer, *target_abs_ms );
    double accuracy_multiplier = get_mortar_accuracy_multiplier( gunner );
    double location_error_cep = get_mortar_location_error( gunner,
                                mortar_base_location_error( location_spotter ) );
    const tripoint_abs_ms location_axis_from = location_spotter.pos;
    tripoint_abs_ms location_axis_to = *target_abs_ms;
    const bool repeat_mission = previous_target && *previous_target == *target_abs_ms;
    if( !repeat_mission ) {
        accuracy_multiplier = mortar_type::skill_accuracy_multiplier( launcher_skill );
        const double base_location_error = mortar_base_location_error( location_spotter );
        if( previous_target ) {
            const double retarget_increase = rl_dist( *previous_target, *target_abs_ms ) *
                                             mortar_near_retarget_location_error_rate(
                                                 location_spotter.observer->get_per() );
    ...
    const mortar_location_error location_error = mortar_make_location_error(
                location_spotter, *target_abs_ms, location_error_cep );
    ...
    const tripoint_abs_ms selected_creeping_axis_to = ( repeat_mission && stored_creeping_axis_to ) ?
    ...
    const tripoint_abs_ms &fire_center_abs_ms = fire_solution.fire_center;
    const std::optional<mortar_creeping_solution> &creeping_solution =
        fire_solution.creeping_solution;
    ...
    const bool existing_feedback_pending = mortar_spotting_feedback_pending( gunner,
                                           *target_abs_ms );
    struct mortar_spotting_roll {
        const mortar_spotter_context *spotter = nullptr;
        double shot_lost_chance = 0.0;
        int roll = 0;
        bool observed = false;
    ...
    bool player_correction_reported = false;
    bool drone_correction_reported = false;
    ...
    const time_duration flight_time = mortar_round_npc_flight_time( mortar_data, round,
                                      target_distance );
    for( int i = 0; i < round_count; ++i ) {
        ...
        const tripoint_abs_ms impact_abs_ms = mortar_data.roll_impact( fire_center_abs_ms,
                                              ...
                                              location_error, ballistic_error, &aimpoint_abs_ms,
                                              mortar_round_max_range( mortar_data, round ) );
        const bool no_wait_adjustment = existing_feedback_pending || i > 0;
        std::vector<mortar_spotting_roll> spotting_rolls;
        bool player_observed = false;
        int player_report_mode = mortar_report_mode_none;
        for( const mortar_spotter_context &spotter : spotters ) {
            const double shot_lost_chance = mortar_shot_lost_chance( spotter, impact_abs_ms );
            const int shot_lost_roll = rng( 1, 10000 );
            const bool shot_observed = shot_lost_roll > shot_lost_chance * 10000.0;
            const bool correction_reported = shot_observed &&
                                             spotter.report_mode != mortar_report_mode_none;
            const double base_feedback_accuracy_multiplier =
                mortar_repeat_accuracy_multiplier( mortar_data, spotter, launcher_skill );
            ...
            const double feedback_location_multiplier = mortar_feedback_multiplier(
                        spotter.repeat_location_multiplier *
                        ( eplrs_net_used ? mortar_eplrs_location_multiplier : 1.0 ),
                        no_wait_adjustment );
            spotting_rolls.emplace_back( mortar_spotting_roll{
                &spotter,
                shot_lost_chance,
                shot_lost_roll,
                shot_observed,
            ...
            if( spotter.player ) {
                player_observed = shot_observed;
                player_correction_reported = player_correction_reported || correction_reported;
                player_report_mode = spotter.report_mode;
            } else if( spotter.drone ) {
                drone_correction_reported = drone_correction_reported || correction_reported;
            }
        }
        const int impact_message_strength = player_report_mode +
                                            ( player_observed ? 0 : mortar_report_lost_offset );
        ...
        add_msg_debug( debugmode::DF_NPC,
                       ...
                       "location spotter %s, rangefinder %d, EPLRS %d, no-wait adjustment %d, "
                       "aimpoint offset %d:%d, impact offset %d:%d.",
                       ...
                       round_is_he ? 1 : 0,
                       location_spotter.name,
                       laser_rangefinder_used ? 1 : 0,
                       eplrs_net_used ? 1 : 0,
                       no_wait_adjustment ? 1 : 0,
                       aimpoint_abs_ms.x() - target_abs_ms->x(),
                       aimpoint_abs_ms.y() - target_abs_ms->y(),
                       impact_abs_ms.x() - target_abs_ms->x(),
                       impact_abs_ms.y() - target_abs_ms->y() );
        for( const mortar_spotting_roll &roll : spotting_rolls ) {
            add_msg_debug( debugmode::DF_NPC,
                           "Mortar spotting by %s for %s round %d/%d: drone %d, pos %d:%d:%d, "
                           "perception %d, sensor multiplier %.2f, detection range multiplier %.2f, "
                           "lost chance %.2f%%, roll %d, observed %d, report mode %d, "
                           "correction reported %d, feedback multiplier %.2f:%.2f.",
                           roll.spotter->name, gunner.disp_name(), i + 1, round_count,
                           roll.spotter->drone ? 1 : 0,
                           roll.spotter->pos.x(), roll.spotter->pos.y(), roll.spotter->pos.z(),
                           roll.spotter->observer->get_per(), roll.spotter->sensor_multiplier,
                           roll.spotter->detection_range_multiplier, roll.shot_lost_chance * 100.0,
                           roll.roll, roll.observed ? 1 : 0, roll.spotter->report_mode,
                           roll.correction_reported ? 1 : 0, roll.feedback_accuracy_multiplier,
                           roll.feedback_location_multiplier );
        }
        ...
        bool feedback_reported = false;
        double feedback_accuracy_multiplier = 1.0;
        double feedback_location_multiplier = 1.0;
        for( const mortar_spotting_roll &roll : spotting_rolls ) {
            if( !roll.correction_reported ) {
                continue;
            }
            feedback_reported = true;
            feedback_accuracy_multiplier *= roll.feedback_accuracy_multiplier;
            feedback_location_multiplier *= roll.feedback_location_multiplier;
        }
        ...
        fire_data.feedback_reported = feedback_reported;
        get_timed_events().add_mortar_fire( fire_time, impact_abs_ms, gunner.disp_name(),
                                            mortar_fire_event_key( gunner ), fire_data );
    }
    add_msg_debug( debugmode::DF_NPC,
                   "Mortar fire mission for %s scheduled %d/%d rounds; player correction pending %d, "
                   "drone correction pending %d.",
                   gunner.disp_name(), round_count, round_count,
                   player_correction_reported ? 1 : 0, drone_correction_reported ? 1 : 0 );

    if( creeping_solution ) {
        add_msg_debug( debugmode::DF_NPC,
                       "Mortar creeping adjustment for %s: center offset %d:%d, heading %03d, "
                       "danger close %d, offset multiplier %.2f, range limited %d.",
                       ...
                       creeping_solution->range_limited ? 1 : 0 );
    }
    ...
    if( creeping_solution ) {
        ...
        const std::string offset_multiplier_text = string_format( "%.1f",
                creeping_solution->offset_multiplier );
        if( creeping_solution->range_limited && creeping_solution->danger_close ) {
            add_msg( _( "%1$s reports Creeping Adjustment offset heading %2$s degrees, range-limited offset.  Danger close to your position." ),
                     gunner.disp_name(), offset_heading_text );
        } else if( creeping_solution->range_limited ) {
            add_msg( _( "%1$s reports Creeping Adjustment offset heading %2$s degrees, range-limited offset." ),
                     gunner.disp_name(), offset_heading_text );
        } else if( creeping_solution->danger_close ) {
...
void select_mortar_ammo_impl( npc &gunner )
{
    ...
    if( selected && *selected != ammo_id ) {
        set_mortar_accuracy_multiplier( gunner, get_mortar_accuracy_multiplier( gunner ) +
                                        mortar_ammo_change_accuracy_penalty( gunner ) );
    }
...
void request_mortar_fire_for_effect_impl( npc &gunner )
{
    ...
    request_mortar_fire_impl( gunner, true, *round_count );
}

talk_effect_fun_t::func f_request_mortar_fire_for_effect()
{
    return []( dialogue const & d ) {
        npc *gunner = d.actor( true )->get_npc();
        if( gunner == nullptr ) {
            debugmsg( "Trying to request mortar fire for effect, but beta talker is not an NPC.  %s",
                      d.get_callstack() );
            return;
        }
        request_mortar_fire_for_effect_impl( *gunner );
    };
}
...
static itype_id fpv_drone_item_id( const std::string &drone_type )
{
    ...
    if( drone_type == "military_suicide" ) {
        return itype_fpv_military_suicide_drone;
    }
...
struct fpv_drone_capabilities {
    int control_range_tiles = 0;
    int endurance_seconds = 0;
    int cruise_speed_tiles_per_hour = 0;
    int launch_delay_multiplier = 0;
    int battery_capacity = 0;
    int payload_capacity_grams = 0;
};

static bool read_positive_fpv_property( const item &drone, const std::string &property,
                                        int &result, std::string &failure )
{
    const int64_t value = drone.get_property_int64_t( property );
    if( value <= 0 || value > INT_MAX ) {
        failure = string_format( "%s has invalid %s", drone.typeId().str(), property );
        return false;
    }
...
static std::optional<fpv_drone_capabilities> read_fpv_drone_capabilities(
    const item &drone, std::string &failure )
{
    fpv_drone_capabilities result;
    if( !read_positive_fpv_property( drone, "fpv_control_range_tiles",
                                     result.control_range_tiles, failure ) ||
        !read_positive_fpv_property( drone, "fpv_endurance_seconds", result.endurance_seconds,
                                     failure ) ||
        !read_positive_fpv_property( drone, "fpv_cruise_speed_tiles_per_hour",
                                     result.cruise_speed_tiles_per_hour, failure ) ||
        !read_positive_fpv_property( drone, "fpv_launch_delay_multiplier",
                                     result.launch_delay_multiplier, failure ) ) {
        return std::nullopt;
    }
    result.battery_capacity = drone.ammo_capacity( ammo_battery );
    if( result.battery_capacity <= 0 ) {
        failure = string_format( "%s has no battery capacity", drone.typeId().str() );
        return std::nullopt;
    }
    if( is_fpv_baba_yaga_drone( drone ) &&
        !read_positive_fpv_property( drone, "fpv_payload_capacity_grams",
                                     result.payload_capacity_grams, failure ) ) {
        return std::nullopt;
    }
    const explosion_data &explosion = drone.type->explosion;
    if( ( is_fpv_suicide_drone( drone ) || is_fpv_military_suicide_drone( drone ) ) &&
        ( explosion.power <= 0.0f || explosion.distance_factor <= 0.0f ||
          explosion.distance_factor >= 1.0f ) ) {
        failure = string_format( "%s has no terminal explosion", drone.typeId().str() );
        return std::nullopt;
    }
    return result;
}

static int fpv_drone_battery_seconds_from_charge( const int charge, const int capacity,
        const int full_battery_seconds )
{
    if( charge <= 0 || capacity <= 0 || full_battery_seconds <= 0 ) {
        return 0;
    }
    return static_cast<int>( std::floor( static_cast<double>( charge ) * full_battery_seconds /
        ...
        const int capacity, const int full_battery_seconds )
{
    ...
    return static_cast<int>( std::ceil( static_cast<double>( battery_seconds ) * capacity /
...
static int fpv_mission_remaining_battery_seconds( const npc &operator_npc, const int at_turn )
{
    ...
    return fpv_drone_battery_seconds_from_charge( remaining_charges, capacity,
            active_fpv_full_battery_seconds( operator_npc ) );
}
...
static int fpv_drone_launch_delay_seconds( const npc &operator_npc, const int multiplier )
{
    const int base_delay = std::max( 0, 30 - 2 * static_cast<int>( operator_npc.get_skill_level(
                                         skill_driving ) ) );
...
static int get_fpv_turn_value( const npc &operator_npc, const std::string &key )
{
    return support_value_int( operator_npc, key );
}

static bool fpv_payload_has_immediate_explosion( const item &payload,
        const float minimum_power = 0.0f )
{
...
static bool fpv_payload_drops_active_item( const item &payload )
{
    return payload.ammo_data() && payload.ammo_data()->ammo->drop_active &&
           !payload.ammo_data()->ammo->drop.is_null();
}
...
static bool is_fpv_baba_yaga_payload( const item &it )
{
    ...
    if( it.has_flag( json_flag_GRENADE ) && fpv_payload_arming_transform( *it.type ) != nullptr ) {
        return true;
    }
    if( it.ammo_type() == ammotype_40x46mm &&
        ( fpv_payload_has_immediate_explosion( it ) || fpv_payload_drops_active_item( it ) ) ) {
        return true;
    }
    if( it.ammo_type() == ammotype_40x53mm && fpv_payload_has_immediate_explosion( it ) ) {
        return true;
    }
...
{
    return count_support_items( operator_npc, [&payload_id]( const item & it ) {
...
{
    const std::string payload_type = support_value_string( operator_npc, "fpv_payload_type" );
...
static int max_baba_yaga_payload_count( const itype_id &payload_id,
                                        const int payload_capacity_grams )
{
    item payload( payload_id, calendar::turn, 1 );
    if( payload.weight() <= 0_gram ) {
        return 0;
    }
    const int64_t payload_count = units::from_gram( payload_capacity_grams ) / payload.weight();
    return static_cast<int>( std::min<int64_t>( payload_count, INT_MAX ) );
}
...
static int load_baba_yaga_payload( npc &operator_npc, const int payload_capacity_grams )
{
    ...
    const int to_load = std::min( fpv_payload_count( operator_npc, *payload_id ),
                                  max_baba_yaga_payload_count( *payload_id,
...
static bool fpv_scout_task_ready( const npc &operator_npc )
{
    return support_value_string( operator_npc, "fpv_scout_active" ) == "yes" &&
           current_turn_number() >= get_fpv_turn_value( operator_npc, "fpv_scout_ready_turn" );
}

static void preserve_ready_fpv_scout_report( npc &operator_npc )
{
    if( !fpv_scout_task_ready( operator_npc ) ) {
        return;
    }
    operator_npc.set_value( "fpv_scout_report_active", "yes" );
    operator_npc.set_value( "fpv_scout_report_x", get_fpv_turn_value( operator_npc, "fpv_scout_x" ) );
    operator_npc.set_value( "fpv_scout_report_y", get_fpv_turn_value( operator_npc, "fpv_scout_y" ) );
...
static bool valid_fpv_mission_state( const npc &operator_npc, std::string &failure )
{
    ...
    const std::optional<fpv_drone_capabilities> capabilities = read_fpv_drone_capabilities(
                *operator_npc.fpv_active_drone, failure );
    if( !capabilities ) {
        return false;
    }
    ...
    if( battery_capacity != capabilities->battery_capacity ||
        full_battery_seconds != capabilities->endurance_seconds ||
...
static void cancel_fpv_assignment_for_new_duty( npc &operator_npc )
{
    ...
    if( operator_npc.has_player_activity() ) {
...
talk_effect_fun_t::func f_reconcile_fpv_mission()
{
    return []( dialogue const & d ) {
        npc *operator_npc = d.actor( true )->get_npc();
        if( operator_npc == nullptr ) {
            debugmsg( "Trying to reconcile drone mission, but beta talker is not an NPC.  %s",
                      d.get_callstack() );
            return;
        }
        reconcile_fpv_mission( *operator_npc );
...
static void schedule_fpv_status_messages( const npc &operator_npc,
        const tripoint_abs_ms &player_pos,
        const std::string &mission_key, const int first_offset_seconds, const int station_seconds_remaining,
        const int total_battery_seconds )
{
    static const std::array<int, 3> status_threshold_percents = { { 20, 10, 5 } };
    std::set<int> scheduled_remaining;
    for( const int percent : status_threshold_percents ) {
        const int remaining = std::max( 1,
                                        static_cast<int>( std::lround( total_battery_seconds * percent / 100.0 ) ) );
        if( remaining >= station_seconds_remaining || !scheduled_remaining.insert( remaining ).second ) {
            continue;
        }
        const int elapsed = station_seconds_remaining - remaining;
        if( elapsed <= 0 ) {
            continue;
        }
        get_timed_events().add( timed_event_type::FPV_DRONE_STATUS_MESSAGE,
                                ...
                                -1, player_pos, remaining, operator_npc.disp_name(),
                                mission_key );
    }
}
...
static bool require_fpv_radio_link( const dialogue &d, const npc &operator_npc )
{
    ...
    if( !you.cache_has_item_with_flag( json_flag_TWO_WAY_RADIO, true ) ||
        ( !operator_npc.cache_has_item_with_flag( json_flag_TWO_WAY_RADIO, true ) &&
          !support_has_item_with_flag( operator_npc, json_flag_TWO_WAY_RADIO ) ) ) {
...
static bool require_fpv_on_station( npc &operator_npc )
{
    ...
    if( status == "enroute" && now < arrival_turn ) {
        add_msg( _( "%1$s reports the drone is still inbound, ETA %2$s." ),
                 operator_npc.disp_name(), format_fpv_duration( arrival_turn - now ) );
        return false;
    }
...
static std::optional<fpv_drone_capabilities> active_fpv_drone_capabilities(
    const npc &operator_npc )
{
    if( !operator_npc.fpv_active_drone ) {
        return std::nullopt;
    ...
    std::string failure;
    std::optional<fpv_drone_capabilities> capabilities = read_fpv_drone_capabilities(
                *operator_npc.fpv_active_drone, failure );
    if( !capabilities ) {
        debugmsg( "Unable to read active FPV capabilities for %s: %s.",
                  operator_npc.disp_name(), failure );
    }
    return capabilities;
}

static int fpv_travel_seconds( const int cruise_speed, const tripoint_abs_ms &from,
...
static bool require_fpv_target_in_range( const npc &operator_npc,
        const tripoint_abs_ms &target, const int max_range )
{
...
{
    const std::optional<fpv_drone_capabilities> capabilities = active_fpv_drone_capabilities(
                operator_npc );
    if( !capabilities ) {
        return std::nullopt;
    }
    if( !require_fpv_target_in_range( operator_npc, target,
                                      capabilities->control_range_tiles ) ) {
        return std::nullopt;
    }
    ...
    plan.travel_seconds = fpv_travel_seconds( capabilities->cruise_speed_tiles_per_hour,
                          station, target );
    plan.delay_seconds = std::max( 1, handling_seconds ) + plan.travel_seconds;
    plan.busy_until = current_turn_number() + plan.delay_seconds;
    plan.return_seconds = fpv_travel_seconds( capabilities->cruise_speed_tiles_per_hour, target,
                          operator_npc.pos_abs() );
        ...
        const tripoint_abs_ms &target, const int handling_seconds )
{
    ...
    const tripoint_abs_ms station( get_fpv_turn_value( operator_npc, "fpv_station_x" ),
                                   get_fpv_turn_value( operator_npc, "fpv_station_y" ),
                                   get_fpv_turn_value( operator_npc, "fpv_station_z" ) );
    const int travel_seconds = fpv_travel_seconds( capabilities->cruise_speed_tiles_per_hour,
                               station, target );
        ...
        const bool track_creatures )
{
    ...
    if( Character *const target_as_character = dynamic_cast<Character *>( target_creature ) ) {
        target.kind = fpv_designation_target_kind::character;
        target.target_character = target_as_character->getID();
    } else if( monster *const target_as_monster = dynamic_cast<monster *>( target_creature ) ) {
        target.kind = fpv_designation_target_kind::monster;
        target.target_monster = get_creature_tracker().temporary_id( *target_as_monster );
    }
    return target;
}

static std::optional<fpv_designation_target> select_fpv_designation_target(
    const std::string &prompt, const std::string &los_failure,
    const bool track_creatures = true )
{
    add_msg( m_info, prompt );
    avatar &you = get_avatar();
    map &here = get_map();
    target_handler::trajectory trajectory = target_handler::mode_select_only( you,
                                            MAX_VIEW_DISTANCE );
    if( trajectory.empty() ) {
        return std::nullopt;
    }
    const tripoint_bub_ms target_bub = trajectory.back();
    if( !here.inbounds( target_bub ) || !you.sees( here, target_bub ) ) {
...
{
    if( support_value_string( operator_npc, "fpv_expend_practiced" ) == "yes" ) {
        return;
    }
...
static int transfer_baba_yaga_payload( npc &operator_npc )
{
    int transferred = 0;
    std::optional<itype_id> selected_id;
    drop_locations payloads = select_nearby_handover_items( is_fpv_baba_yaga_payload,
                              _( "Select bomber drone payloads to hand over" ) );
    for( drop_location &selected : payloads ) {
        std::optional<item> payload = take_handover_item( selected.first, selected.second );
        if( !payload || !is_fpv_baba_yaga_payload( *payload ) ) {
            continue;
        }
        const int payload_count = fpv_payload_item_count( *payload );
        if( payload_count <= 0 ) {
            continue;
        }
...
{
    return is_fpv_suicide_drone( it ) || is_fpv_military_suicide_drone( it ) ||
           is_fpv_scout_drone( it ) || is_fpv_baba_yaga_drone( it ) ||
           it.typeId() == itype_remotevehcontrol || it.has_flag( json_flag_TWO_WAY_RADIO ) ||
           is_fpv_baba_yaga_payload( it );
}

talk_effect_fun_t::func f_manage_fpv_support_inventory()
{
    return []( dialogue const & d ) {
        npc *operator_npc = d.actor( true )->get_npc();
        if( operator_npc == nullptr ) {
            debugmsg( "Trying to manage drone support inventory, but beta talker is not an NPC.  %s",
                      d.get_callstack() );
            return;
        }
        if( d.by_radio ) {
        ...
        manage_support_inventory( *operator_npc, is_fpv_support_item, _( "Drone support equipment" ),
                                  _( "Select drone equipment to hand over" ),
                                  _( "Take back which drone support equipment?" ) );
    };
}
...
talk_effect_fun_t::func f_assign_fpv_drone_operator()
{
    return []( dialogue const & d ) {
        ...
        if( operator_npc->get_skill_level( skill_driving ) < 2 ) {
            add_msg( _( "%s needs at least moderate driving training to pilot drones." ),
                     operator_npc->disp_name() );
            return;
        }
        if( operator_npc->has_player_activity() ) {
        ...
        if( needs_controller ) {
            item_location controller = select_nearby_handover_item( []( const item & it ) {
                return it.typeId() == itype_remotevehcontrol;
            }, _( "Select a remote vehicle controller to hand over" ) );
            std::optional<item> controller_item = take_handover_item( controller, 1 );
            if( !controller_item || controller_item->typeId() != itype_remotevehcontrol ) {
        ...
        int military_suicide_drones = 0;
        int scout_drones = 0;
        int baba_yaga_drones = 0;
        const auto drone_filter = []( const item & it ) {
            return is_fpv_suicide_drone( it ) || is_fpv_military_suicide_drone( it ) ||
                   is_fpv_scout_drone( it ) || is_fpv_baba_yaga_drone( it );
        };
        drop_locations drones = select_nearby_handover_items( drone_filter,
                                _( "Select drones to hand over" ) );
        for( drop_location &selected : drones ) {
            std::optional<item> drone = take_handover_item( selected.first, selected.second );
            if( !drone ) {
                continue;
            }
        ...
        const int payload_count = ( baba_yaga_drones > 0 ||
                                  ...
                                  transfer_baba_yaga_payload( *operator_npc ) : 0;
        operator_npc->set_value( "fpv_assignment", "operator" );
        clear_fpv_mission( *operator_npc );
        operator_npc->assign_activity( operate_drone_activity_actor() );
        g->add_npc_follower( operator_npc->getID() );
        operator_npc->chatbin.first_topic = operator_npc->chatbin.talk_friend;
        operator_npc->guard_pos = std::nullopt;
        operator_npc->clear_ai_guard_pos();
        operator_npc->goal = npc::no_goal_point;
        operator_npc->omt_path.clear();
        operator_npc->path.clear();
        operator_npc->chair_pos = std::nullopt;
        operator_npc->wander_pos = std::nullopt;
        operator_npc->clear_destination();
        operator_npc->clear_committed_goal();

        const int drone_count = suicide_drones + military_suicide_drones + scout_drones + baba_yaga_drones;
        if( drone_count == 0 ) {
            add_msg( _( "%s sets up as drone operator, but still needs drones." ),
                     operator_npc->disp_name() );
        } else if( baba_yaga_drones > 0 && payload_count > 0 ) {
...
talk_effect_fun_t::func f_unassign_fpv_drone_operator()
{
    return []( dialogue const & d ) {
        npc *operator_npc = d.actor( true )->get_npc();
        if( operator_npc == nullptr ) {
            debugmsg( "Trying to unassign drone operator, but beta talker is not an NPC.  %s",
                      d.get_callstack() );
            return;
        }
        if( d.by_radio ) {
...
static void return_unlaunched_fpv_drone( npc &operator_npc, item drone )
{
    std::optional<item> rejected = operator_npc.stow_fpv_support_item( std::move( drone ) );
    if( rejected ) {
...
static void request_fpv_launch( dialogue const &d, const std::string &drone_type )
{
    ...
    std::string capability_failure;
    const std::optional<fpv_drone_capabilities> capabilities = read_fpv_drone_capabilities(
                *launched_drone, capability_failure );
    if( !capabilities ) {
        debugmsg( "Unable to launch %s: %s.", launched_drone->typeId().str(), capability_failure );
        add_msg( _( "%s reports that the drone has an invalid flight configuration." ),
                 operator_npc->disp_name() );
        return_unlaunched_fpv_drone( *operator_npc, std::move( *launched_drone ) );
        return;
    }
    ...
    const int max_range_tiles = capabilities->control_range_tiles;
    if( distance > max_range_tiles ) {
        return_unlaunched_fpv_drone( *operator_npc, std::move( *launched_drone ) );
        if( scout_drone ) {
            add_msg( _( "%1$s reports that you are outside the scout drone's %2$d tile control range." ),
                     operator_npc->disp_name(), max_range_tiles );
        } else if( baba_yaga_drone ) {
            add_msg( _( "%1$s reports that you are outside the bomber drone's %2$d tile control range." ),
                     operator_npc->disp_name(), max_range_tiles );
        } else if( drone_type == "military_suicide" ) {
            add_msg( _( "%1$s reports that you are outside the military explosive FPV's %2$d tile control range." ),
                     operator_npc->disp_name(), max_range_tiles );
        } else {
            add_msg( _( "%1$s reports that you are outside the FPV drone's %2$d tile control range." ),
                     operator_npc->disp_name(), max_range_tiles );
        }
        return;
    }

    const int cruise_seconds = std::max( 1, fpv_travel_seconds(
                                         capabilities->cruise_speed_tiles_per_hour,
                                         operator_npc->pos_abs(), you.pos_abs() ) );
    const int outbound_seconds = cruise_seconds + fpv_drone_launch_delay_seconds( *operator_npc,
                                 capabilities->launch_delay_multiplier );
    const int return_seconds = cruise_seconds;
    const int battery_capacity = capabilities->battery_capacity;
    const int battery_charges = clamp( launched_drone->ammo_remaining(), 0, battery_capacity );
    const int battery_seconds = fpv_drone_battery_seconds_from_charge( battery_charges,
                                battery_capacity, capabilities->endurance_seconds );
    ...
    const std::string mission_key = string_format( "fpv_%d_%d", operator_npc->getID().get_value(),
                                    now );
    const int payload_loaded = baba_yaga_drone ? load_baba_yaga_payload(
                               *operator_npc, capabilities->payload_capacity_grams ) : 0;
    ...
    operator_npc->set_value( "fpv_battery_full_seconds", capabilities->endurance_seconds );
    ...
    const tripoint_abs_ms station_abs = you.pos_abs();
    operator_npc->set_value( "fpv_station_x", station_abs.x() );
    operator_npc->set_value( "fpv_station_y", station_abs.y() );
    operator_npc->set_value( "fpv_station_z", station_abs.z() );
    ...
    schedule_fpv_status_messages( *operator_npc, you.pos_abs(), mission_key, outbound_seconds,
                                  station_seconds, battery_seconds );
    get_timed_events().add( timed_event_type::FPV_DRONE_RETURN_MESSAGE,
    ...
    if( scout_drone ) {
        add_msg( _( "You authorize scout drone launch.  %1$s reports ETA %2$s, planned time on station %3$s, and recovery %4$s after return." ),
                 operator_npc->disp_name(), format_fpv_duration( outbound_seconds ),
                 format_fpv_duration( station_seconds ), format_fpv_duration( return_seconds ) );
    } else if( baba_yaga_drone ) {
        if( payload_loaded > 0 ) {
            add_msg( _( "You authorize bomber drone launch.  %1$s reports ETA %2$s, planned time on station %3$s, recovery %4$s after return, and %5$d payload aboard." ),
                     operator_npc->disp_name(), format_fpv_duration( outbound_seconds ),
                     format_fpv_duration( station_seconds ), format_fpv_duration( return_seconds ),
                     payload_loaded );
        } else {
            add_msg( _( "You authorize bomber drone launch.  %1$s reports ETA %2$s, planned time on station %3$s, and no payload aboard." ),
                     operator_npc->disp_name(), format_fpv_duration( outbound_seconds ),
                     format_fpv_duration( station_seconds ) );
        }
    } else if( drone_type == "military_suicide" ) {
        add_msg( _( "You authorize military explosive FPV launch.  %1$s reports ETA %2$s, planned time on station %3$s, and recovery %4$s after return." ),
                 operator_npc->disp_name(), format_fpv_duration( outbound_seconds ),
                 format_fpv_duration( station_seconds ), format_fpv_duration( return_seconds ) );
    } else {
        add_msg( _( "You authorize FPV launch.  %1$s reports ETA %2$s, planned time on station %3$s, and recovery %4$s after return." ),
                 operator_npc->disp_name(), format_fpv_duration( outbound_seconds ),
                 format_fpv_duration( station_seconds ), format_fpv_duration( return_seconds ) );
    }
}
...
talk_effect_fun_t::func f_request_fpv_one_way()
{
    return []( dialogue const & d ) {
        ...
        if( support_value_string( *operator_npc, "fpv_one_way" ) == "yes" ) {
...
talk_effect_fun_t::func f_request_fpv_abort_one_way()
{
    return []( dialogue const & d ) {
        npc *operator_npc = d.actor( true )->get_npc();
        if( operator_npc == nullptr ) {
            debugmsg( "Trying to abort drone one-way loiter, but beta talker is not an NPC.  %s",
                      d.get_callstack() );
            return;
        }
        if( !require_fpv_radio_link( d, *operator_npc ) || !require_fpv_controller( *operator_npc ) ) {
        ...
        const std::string status = support_value_string( *operator_npc, "fpv_status" );
        if( status != "on_station" && status != "enroute" ) {
            add_msg( _( "%s reports no drone is airborne." ), operator_npc->disp_name() );
            return;
        }
        if( support_value_string( *operator_npc, "fpv_one_way" ) != "yes" ) {
            add_msg( _( "%s reports the drone is already set to return on bingo fuel." ),
                     operator_npc->disp_name() );
            return;
        }
        ...
        int station_seconds = 0;
        if( remaining_cruise_seconds > return_seconds + reserve_seconds ) {
            station_seconds = std::max( 0, static_cast<int>( std::floor(
                                            ( remaining_cruise_seconds - return_seconds - reserve_seconds ) / 0.6 ) ) );
        }
        const int station_end_turn = start_turn + station_seconds;
        const int return_end_turn = station_end_turn + return_seconds;
        const std::string mission_key = support_value_string( *operator_npc, "fpv_mission_key" );
        avatar &you = get_avatar();
        ...
        if( start_turn > now ) {
            add_msg( _( "You order the drone back to bingo-return behavior.  %1$s confirms current task completion first; return profile resumes in %2$s." ),
                     operator_npc->disp_name(), format_fpv_duration( start_turn - now ) );
        } else {
            add_msg( _( "You order the drone back to bingo-return behavior.  %1$s confirms." ),
                     operator_npc->disp_name() );
        }
    };
}

talk_effect_fun_t::func f_request_fpv_recover()
{
    return []( dialogue const & d ) {
        npc *operator_npc = d.actor( true )->get_npc();
        if( operator_npc == nullptr ) {
            debugmsg( "Trying to request drone recovery, but beta talker is not an NPC.  %s",
                      d.get_callstack() );
            return;
        }
        if( !require_fpv_radio_link( d, *operator_npc ) || !require_fpv_controller( *operator_npc ) ) {
        ...
        if( status != "on_station" ) {
            add_msg( _( "%s reports no drone is available for recovery order." ),
                     operator_npc->disp_name() );
            return;
        }
        if( support_value_string( *operator_npc, "fpv_one_way" ) == "yes" ) {
            add_msg( _( "%s reports the drone is committed one-way; use the bingo-return abort order first." ),
                     operator_npc->disp_name() );
            return;
        }
        ...
        if( start_turn > now ) {
            ...
            add_msg( _( "You order immediate drone recovery.  %1$s confirms return profile is active." ),
                     operator_npc->disp_name() );
        }
    };
}
...
talk_effect_fun_t::func f_request_fpv_attack()
{
    return []( dialogue const & d ) {
        ...
        if( now < arrival_turn ) {
            add_msg( _( "%1$s reports the FPV drone is still inbound, ETA %2$s." ),
                     operator_npc->disp_name(), format_fpv_duration( arrival_turn - now ) );
            return;
        }
        if( now > station_end_turn ) {
        ...
        if( !operator_npc->fpv_active_drone ||
            operator_npc->fpv_active_drone->type->explosion.power <= 0.0f ) {
            debugmsg( "Active terminal FPV drone has no item-defined explosion." );
            add_msg( _( "%s reports the active drone's terminal payload is invalid." ),
                     operator_npc->disp_name() );
            return;
        }
        const explosion_data drone_explosion = operator_npc->fpv_active_drone->type->explosion;

        map &here = get_map();
        const std::optional<fpv_designation_target> drone_designated_target =
            active_fpv_designation( "fpv_attack" );
        uilist target_menu;
        target_menu.text = _( "Command FPV drone to attack what?" );
        target_menu.addentry( 0, true, 'l', _( "Local target" ) );
        target_menu.addentry( 1, drone_designated_target.has_value(), 'd',
                              drone_designated_target ? _( "Drone-designated target" ) :
        ...
        std::optional<fpv_designation_target> target;
        if( target_mode == 1 ) {
            target = drone_designated_target;
        } else {
            target = select_fpv_designation_target(
                         _( "Designate a local FPV attack target." ),
                         _( "You need line of sight to designate that target." ) );
        }
        if( !target ) {
            return;
        }
        ...
        const bool fixed_point_attack = target->kind == fpv_designation_target_kind::tile;
...
talk_effect_fun_t::func f_request_fpv_payload_drop()
{
    return []( dialogue const & d ) {
        ...
        map &here = get_map();
        const std::optional<fpv_designation_target> drone_designated_target =
            active_fpv_designation( "payload_drop" );
        uilist target_menu;
        ...
        if( target_mode == 1 ) {
            ...
            target = select_fpv_designation_target(
                         _( "Designate a local bomber drone payload drop target." ),
                         _( "You need line of sight to designate that drop target." ) );
        }
        if( !target ) {
            return;
        }
        ...
        tripoint_abs_ms impact_abs = apply_circular_cep( target->pos, cep );
...
talk_effect_fun_t::func f_request_fpv_scout_report( const bool thermal )
{
    return [thermal]( dialogue const & d ) {
        ...
        const bool has_saved_report = support_value_string( *operator_npc,
                                      "fpv_scout_report_active" ) == "yes";
...
talk_effect_fun_t::func f_request_fpv_designation()
{
    return []( dialogue const & d ) {
        ...
        write_fpv_designation_target( *operator_npc, designation_type, *target );
        add_msg( _( "%s marks a target from the drone scout feed." ),
                 operator_npc->disp_name() );
    };
}
...
void add_mortar_adjustment_downtime( npc &gunner )
{
    ...
    if( !mortar ) {
        add_msg_debug( debugmode::DF_NPC,
                       "Mortar adjustment downtime ignored for %s: no assigned mortar.",
                       gunner.disp_name() );
        return;
    }

    const time_duration downtime = mortar_adjustment_downtime( gunner, mortar->pos );
    const time_point old_ready_at = mortar_adjustment_ready_at( gunner );
    const time_point new_ready_at = std::max( calendar::turn, old_ready_at ) + downtime;
    set_mortar_adjustment_ready_at( gunner, new_ready_at );
    add_msg_debug( debugmode::DF_NPC,
                   "Mortar adjustment downtime for %s: added %d seconds, ready in %d turns.",
                   gunner.disp_name(), to_seconds<int>( downtime ),
                   to_turns<int>( new_ready_at - calendar::turn ) );
}
                            ...
                            const tripoint_abs_ms &mortar_pos, const itype_id &ammo_id )
{
    ...
    const std::optional<assigned_mortar> assignment = get_assigned_mortar( gunner );
    if( !assignment || assignment->pos != mortar_pos || assignment->type->id != mortar_id ||
        rl_dist( gunner.pos_abs(), mortar_pos ) > 1 ) {
        return false;
    }
    if( gunner.get_skill_level( skill_launcher ) < mortar_type::minimum_launcher_skill() ) {
        return false;
...
void fire_queued_mortar( npc &gunner, const tripoint_abs_ms &target, const int round_count )
{
    if( !get_assigned_mortar( gunner ) ) {
        add_msg_debug( debugmode::DF_NPC,
                       "Queued mortar fire ignored for %s: no assigned mortar.",
                       gunner.disp_name() );
        return;
    }
    request_mortar_fire_impl( gunner, false, std::max( 1, round_count ), target, true );
}
...
#undef WRAP
    ...
    if( effect_id == "request_mortar_fire_for_effect" ) {
        set_effect( talk_effect_fun_t( talk_effect_fun::f_request_mortar_fire_for_effect() ) );
        return;
    }
    ...
    if( effect_id == "unassign_fpv_drone_operator" ) {
        set_effect( talk_effect_fun_t( talk_effect_fun::f_unassign_fpv_drone_operator() ) );
        return;
    }
    if( effect_id == "manage_fpv_support_inventory" ) {
        set_effect( talk_effect_fun_t( talk_effect_fun::f_manage_fpv_support_inventory() ) );
        return;
    }
    if( effect_id == "reconcile_fpv_mission" ) {
        set_effect( talk_effect_fun_t( talk_effect_fun::f_reconcile_fpv_mission() ) );
        return;
    }
    ...
    if( effect_id == "request_fpv_abort_one_way" ) {
        set_effect( talk_effect_fun_t( talk_effect_fun::f_request_fpv_abort_one_way() ) );
        return;
    }
    if( effect_id == "request_fpv_recover" ) {
        set_effect( talk_effect_fun_t( talk_effect_fun::f_request_fpv_recover() ) );
        return;
    }
